	...
	for _, svc := range p.Services {
		...
		for _, meth := range svc.Methods {
			...
			for _, b := range meth.Bindings {
				...
				if err := clientFuncTemplate.Execute(w, binding{Binding: b}); err != nil {
					return "", err
				}
...
func (c* default{{.Method.Service.GetName}}HttpClient) {{.Method.GetName}}(ctx context.Context, in *{{.Method.RequestType.GoType .Method.Service.File.GoPkg.Path}}) (*{{.Method.ResponseType.GoType .Method.Service.File.GoPkg.Path}}, error) {
	...
	// I think a way to implement that, would be recursively go through all the Request Message, and create corresponding query param, but with excludint the one that have been already put in the body, and in the path param...
	// the logic can be implemeted in a method, and not in the template
